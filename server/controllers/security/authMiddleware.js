...
export function authenticateAndAuthorize(requiredPrivileges = [], requiredRole = null, allowSelf = false) {
  return (req, res, next) => {
    // Try fetching token from cookies first, then from headers
    ...
    try {
      ...
      const hasRequiredPrivileges = requiredPrivileges.every((privilege) => userPrivileges.includes(privilege));
