...
export class User {
  constructor(
    ...
    if (!Object.values(UserRoleEnum).includes(role)) {
      throw new Error(
        `Invalid role: ${role}. Allowed roles are ${Object.values(
          UserRoleEnum
        ).join(", ")}.`
      );
    }
  ...
  #_Collection = class Collection {
    ...
    asArray() {
      return this.archive.toArray().includes(this.latest)
        ? this.archive.toArray()
        : [...this.archive.toArray(), this.latest];
    }
  };
  ...
  removeFromCart(product_id) {
    this.shopping_cart = this.shopping_cart.filter(
  ...
  getSentMessages() {
    return this.messages.filter(
      (message) => message.sender_id === this.user_id
    );
  }
